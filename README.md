...
## Конфигурация линтеров в проекте

Каждый линтер автоматически подхватывает свой конфиг-файл из корня репозитория. Создай нужный файл — и линтер применит его без каких-либо изменений в workflow.

| Линтер | Файл конфига | Что можно настроить |
|--------|-------------|---------------------|
| shellcheck | `.shellcheckrc` | отключить правила, задать shell по умолчанию |
| hadolint | `.hadolint.yaml` | игнорировать правила, доверенные registry |
| yamllint | `.yamllint` или `.yamllint.yml` | отступы, длина строки, свои правила |
| markdownlint | `.markdownlintrc` или `.markdownlint.json` | включить/отключить правила |
| gitleaks | `.gitleaks.toml` | allowlist для путей, коммитов, паттернов |

Пример `.hadolint.yaml` — игнорировать конкретные правила:
```yaml
ignore:
  - DL3008
  - DL3018
...
Пример `.shellcheckrc` — отключить предупреждение глобально:
```
disable=SC2086
```

Пример `.gitleaks.toml` — исключить файл из проверки:
```toml
[allowlist]
paths = ['''^test/fixtures/''']
```
